...
加载 `Assets/Models/Candidates_NoCut_Waterbomb_4Module.json` 后，点击
`Auto / Reset` 会按照 STM32 程序的 PWM 顺序循环驱动四组折痕：

1. 舵机 1、3 从 `1350 us` 转到 `1650 us`，等待 `0.5 s`；
2. 舵机 2、4 从 `1350 us` 转到 `1650 us`，保持 `1.2 s`；
3. 舵机 2、4 回到 `1350 us`，等待 `0.5 s`；
4. 舵机 1、3 回到 `1350 us`，保持 `1.2 s` 后重复。

序列运行时再次点击 `Auto / Reset`，会停止循环并将四组舵机全部复位到
`1350 us`。该按钮只用于四模块 B 模型；其他模型缺少对应折痕组时不会启动。
...
- 常规滑条仍统一控制全部折痕；四模块 B 模型可由 `Auto / Reset` 临时接管四组折痕并按舵机顺序独立驱动。
